...
class Linkedin_post extends CI_Controller
{
	 function __construct()
	  {
		...
		$this->load->model('social_accounts/Linkedin_post_model');
	  }	 
	 ...
	 function index()
	 {
		...
		$qr=$this->Linkedin_post_model->get_all_li_rec();
		
		foreach($qr as $row)
		{
			$posts=$this->Linkedin_post_model->get_all_post_li($row['job_id'],$row['sync_frequency']);
			//echo "<pre>";print_r($ss);die;
			$target_li_id=explode(',',$row['li_target_id']);
			...
			foreach($posts as $post){
				...
				foreach($target_li_id as $soc)
				{	
					
					$token=$this->Linkedin_post_model->get_li_token($soc);
					...
					if(@$result['id'])
					 {
						...
						$this->Linkedin_post_model->update_li_status($post['post_id']);
					 }
					else
					 {
					...
					$this->Linkedin_post_model->add_sync_log($row['job_id'],$arr);
			...
			/* $ar=array(
			 ...
			 $token=$this->Linkedin_post_model->get_update_li_time($row['job_id'],$ar);*/
